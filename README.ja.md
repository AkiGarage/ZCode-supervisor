...
現在の public setup path は、この GitHub repo から実行する `uvx` です。
...
最初の PyPI release 後は、短い package command が使えるようになります。
...
2. GitHub-backed `uvx` path を優先してください。`uvx --version` が動く場合は
...
- 初回 setup: `zcode-install-repo /path/to/target-repo`
- route の確認: `zcode-auto-route --workspace ... --objective ...`
- 実際の delegated implementation: Codex が scope と validation を決めてから
  `zcode-auto-route ... --allowed ... --validation ... --execute`
